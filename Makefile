PREFIX = /usr/local
CC = cc
CFLAGS = -O2 -Wall -Wextra -std=c17
LDFLAGS = 
CPPFLAGS = 
CJSON_SRC = 
CJSON_CFLAGS = -I/usr/include/cjson 
CJSON_LIBS = -lcjson 
CURL_CFLAGS = -I/usr/include/x86_64-linux-gnu -I/usr/local/include -isystem /usr/include/mit-krb5 -I/usr/include/p11-kit-1 
CURL_LIBS = -lcurl 
SQLITE_CFLAGS = 
SQLITE_LIBS = -lsqlite3 
BINDIR = $(PREFIX)/bin
MANDIR = $(PREFIX)/share/man/man1
DATADIR = $(PREFIX)/share/ytran

ALL_CFLAGS = $(CPPFLAGS) $(CFLAGS) $(CURL_CFLAGS) $(SQLITE_CFLAGS) $(CJSON_CFLAGS) -DPREFIX=\"$(PREFIX)\"
ALL_LIBS = $(CURL_LIBS) $(SQLITE_LIBS) $(CJSON_LIBS) -lpthread -ldl -lm

.PHONY: all install uninstall clean distclean check dist

all: src/ytran

src/ytran: src/ytran.c $(CJSON_SRC)
	$(CC) $(ALL_CFLAGS) $(LDFLAGS) -o $@ src/ytran.c $(CJSON_SRC) $(ALL_LIBS)

install: src/ytran
	install -d $(DESTDIR)$(BINDIR)
	install -m 755 src/ytran $(DESTDIR)$(BINDIR)/ytran
	install -d $(DESTDIR)$(MANDIR)
	install -m 644 ytran.1 $(DESTDIR)$(MANDIR)/ytran.1
	install -d $(DESTDIR)$(DATADIR)
	install -m 644 examples/models $(DESTDIR)$(DATADIR)/models
	install -m 644 examples/default $(DESTDIR)$(DATADIR)/default

uninstall:
	rm -f $(DESTDIR)$(BINDIR)/ytran
	rm -f $(DESTDIR)$(MANDIR)/ytran.1
	rm -f $(DESTDIR)$(DATADIR)/models
	rm -f $(DESTDIR)$(DATADIR)/default

clean:
	rm -f src/ytran

distclean: clean
	rm -f Makefile

check: src/ytran
	@src/ytran --version
	@src/ytran --help >/dev/null
	@echo "All checks passed."

dist:
	rm -rf ytran-1.0
	mkdir -p ytran-1.0/src/deps/cjson ytran-1.0/contrib
	cp configure Makefile.in requirements.txt ytran-1.0/
	cp contrib/ytran.py ytran-1.0/contrib/
	cp src/ytran.c ytran-1.0/src/
	cp src/deps/cjson/cJSON.c src/deps/cjson/cJSON.h ytran-1.0/src/deps/cjson/
	cp README.md COPYING AUTHORS NEWS ChangeLog INSTALL ytran.1 examples/default examples/models ytran-1.0/
	tar czf ytran-1.0.tar.gz ytran-1.0
	rm -rf ytran-1.0
	@echo "Created ytran-1.0.tar.gz"
